...
- 覆盖 58 个目录城市的离线轨道路线规划。其中 53 个城市包含按 ODbL 1.0 单独署名和授权的
  OpenStreetMap 地铁网络基础数据，共 6,718 座车站；另有 5 个仅保留城市目录信息。
- Apple 地图地点搜索、步行路线和地图渲染。
- 14 个城市共 582 座车站的无障碍数据，来自各城市自己的公开数据。
...
- 面向全部 58 个目录城市的内置官方资源目录，包含 770 个已审核的乘客地图、出行、
  无障碍和帮助链接。
- 配置百度地图密钥后可选的联网增强：票价、实测换乘通道长度、首末班车时间、末班车提醒旁的
  出租车费用估算，以及首末段的骑行与电动车路线。这些内容不内置、不写入磁盘、不提交入库，
  没有它们应用同样可以规划路线。
...
其余 44 个目录城市的车站数据处于来源待接入状态。轨道路线由内置地铁网络计算，不依赖可选城市
数据包。应用不会把当前任何城市包描述为可下载内容。
...
实际展示的内容来自官方开放数据：城市公布出入口时显示出入口。配置在线路线服务后，换乘通道
长度以距离形式给出，而不是一条路径。
...
- 香港实时到站请求使用官方线路和车站标识访问 `rt.data.gov.hk`，不会包含
  乘客位置。
