...
private enum PersonalStationMediaHarness {
    ...
    private static func testSourceLimit() async throws {
        ...
        try await requireMediaError(.sourceTooLarge) {
            try await store.importImageData(oversized, for: key)
        }
    ...
    private static func testNormalizationAndMetadataStripping() async throws {
        ...
        let item = try await store.importImageData(sourceData, for: key)
    ...
    private static func testItemLimit() async throws {
        ...
        for _ in 0..<PersonalStationMediaStore.maximumItemsPerStation {
            try await store.importImageData(sourceData, for: key)
        }
    ...
    private static func testStationIsolation() async throws {
        ...
        let centralItem = try await store.importImageData(sourceData, for: central)
        let admiraltyItem = try await store.importImageData(sourceData, for: admiralty)
        let otherCityItem = try await store.importImageData(sourceData, for: otherCity)
    ...
    private static func testTraversalRecordRecovery() async throws {
        ...
        let validItem = try await initialStore.importImageData(sourceData, for: key)
